...
export class RateLimiter {
  ...
  public async initRedis(redisUrl: string, password?: string): Promise<void> {
    try {
      this.redis = new Redis(redisUrl, {
        ...
        retryStrategy: (times) => {
          if (times > 3) {
            this.logger.error("Redis connection failed after 3 retries, using memory fallback")
            return null
          }
      ...
      this.logger.error("Failed to initialize Redis rate limiter, using memory fallback:", error)
   ...
   */
  private setRateLimitHeaders(res: Response, rateLimiterRes: RateLimiterRes, maxRequests: number): void {
    res.setHeader("X-RateLimit-Limit", maxRequests.toString())
    res.setHeader("X-RateLimit-Remaining", Math.max(0, rateLimiterRes.remainingPoints).toString())
    res.setHeader("X-RateLimit-Reset", new Date(Date.now() + rateLimiterRes.msBeforeNext).toISOString())

    // Retry-After header for limited requests
    if (rateLimiterRes.remainingPoints <= 0) {
      res.setHeader("Retry-After", Math.ceil(rateLimiterRes.msBeforeNext / 1000).toString())
    }
  }
   ...
   */
  public async checkLimit(key: string): Promise<{ allowed: boolean; info: RateLimitInfo | null }> {
    try {
      const limiter = this.limiterRedis || this.limiterMemory
      ...
      const remaining = Math.max(0, this.config.maxRequests - res.consumedPoints)
