...
export class HealthChecker extends EventEmitter {
  ...
  constructor(targets: TargetConfig[], options: HealthCheckerOptions = {}) {
    ...
    this.targets = targets.map((target) => ({
      ...
      responseTime: 0,
      checkInterval: target.healthCheck?.interval || this.defaultOptions.interval,
    }))
  ...
  private async checkTarget(targetHealth: TargetHealth): Promise<void> {
    ...
    try {
      const response = await this.makeHealthCheckRequest(protocol, target.host, port, healthConfig)
      ...
      if (response.healthy) {
        ...
        this.handleTargetFailure(targetHealth, `Unhealthy status: ${response.statusCode}`)
      }
    } catch (error) {
      this.handleTargetFailure(targetHealth, error instanceof Error ? error.message : String(error))
    }
  ...
  private makeHealthCheckRequest(
    ...
    return new Promise((resolve, reject) => {
      ...
      const request = protocol === "https" ? https.request(options) : http.request(options)
   ...
   */
  private handleTargetSuccess(targetHealth: TargetHealth, responseTime: number): void {
    ...
    if (!wasHealthy && targetHealth.consecutiveSuccesses >= this.defaultOptions.successThreshold) {
  ...
  private handleTargetFailure(targetHealth: TargetHealth, error: string): void {
    ...
    if (wasHealthy && targetHealth.consecutiveFailures >= this.defaultOptions.failureThreshold) {
  ...
  public getTargetHealth(target: TargetConfig): TargetHealth | null {
    return this.targets.find((t) => t.target.host === target.host && t.target.port === target.port) || null
  }
  ...
  public getStats(): {
    ...
    const avgResponseTime =
      healthyTargets.length > 0 ? healthyTargets.reduce((sum, t) => sum + t.responseTime, 0) / healthyTargets.length : 0
