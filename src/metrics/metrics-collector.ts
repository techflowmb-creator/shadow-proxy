...
export class MetricsCollector extends EventEmitter {
  ...
  public middleware(req: Request, res: Response, next: NextFunction): void {
    ...
    res.end = ((...args: any[]) => {
      ...
      this.metrics.activeRequests.value = Math.max(
        0,
        this.metrics.activeRequests.value - 1,
      );
      this.metrics.activeRequests.lastUpdated = Date.now();
  ...
  private addResponseTime(duration: number): void {
    ...
    if (this.metrics.responseTime.values.length > this.maxHistogramValues) {
      this.metrics.responseTime.values = this.metrics.responseTime.values.slice(
        -this.maxHistogramValues,
      );
    }
  ...
  public getMetrics(): MetricsSnapshot {
    ...
    const requestsPerSecond =
      (this.metrics.totalRequests.value / windowMs) * 1000;
    const errorsPerSecond = (this.metrics.totalErrors.value / windowMs) * 1000;

    const sortedLatencies = [...this.metrics.responseTime.values].sort(
      (a, b) => a - b,
    );
    ...
    return {
      ...
      throughput: {
        ...
        bytesInPerSecond: Math.round(
          (this.metrics.bytesIn.value / uptime) * 1000,
        ),
        bytesOutPerSecond: Math.round(
          (this.metrics.bytesOut.value / uptime) * 1000,
        ),
      },
      errors: {
  ...
  public getPrometheusMetrics(): string {
    ...
    lines.push(
      `shadow_request_duration_seconds{quantile="0.5"} ${metrics.latency.p50 / 1000}`,
    );
    lines.push(
      `shadow_request_duration_seconds{quantile="0.95"} ${metrics.latency.p95 / 1000}`,
    );
    lines.push(
      `shadow_request_duration_seconds{quantile="0.99"} ${metrics.latency.p99 / 1000}`,
    );
    ...
    lines.push(
      `shadow_bytes_total{direction="in"} ${metrics.throughput.bytesIn}`,
    );
    lines.push(
      `shadow_bytes_total{direction="out"} ${metrics.throughput.bytesOut}`,
    );
