...
export class ProxyHandler {
  ...
  private activeConnections: Map<string, { target: TargetConfig; startTime: number }> = new Map()
  private requestTransforms: Map<string, (req: Request) => RequestTransform | Promise<RequestTransform>> = new Map()
  private responseTransforms: Map<
    string,
    (res: Response, body: Buffer) => ResponseTransform | Promise<ResponseTransform>
  > = new Map()
  private wsServer: WebSocketServer | null = null

  constructor(config: ServerConfig, cacheManager: CacheManager, loadBalancer: LoadBalancer) {
    this.config = config
  ...
  private setupProxyEvents(): void {
    ...
    this.proxy.on("proxyRes", (proxyRes: http.IncomingMessage, req: any, res: any) => {
    ...
    this.proxy.on("proxyReq", (proxyReq: http.ClientRequest, req: any, res: any) => {
      const requestId = req.headers["x-request-id"] || this.generateRequestId()
  ...
  public async handle(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      if (this.config.cache.enabled && this.cacheManager.isCacheableMethod(req.method)) {
      ...
      this.activeConnections.set(connectionId, { target, startTime: Date.now() })
  ...
  private async checkCache(req: Request, res: Response): Promise<boolean> {
    try {
      const cacheKey = this.cacheManager.generateKey(req.method, req.originalUrl, JSON.stringify(req.body))
   ...
   * Cache the response
   */
  private async cacheResponse(proxyRes: http.IncomingMessage, req: any, res: any): Promise<void> {
   ...
   */
  private async applyResponseTransform(proxyRes: http.IncomingMessage, req: any, res: any): Promise<void> {
    for (const [pattern, transform] of this.responseTransforms) {
      if (this.matchesPattern(req.url, pattern)) {
        try {
  ...
  public registerResponseTransform(
    pattern: string,
    transform: (res: Response, body: Buffer) => ResponseTransform | Promise<ResponseTransform>,
  ): void {
